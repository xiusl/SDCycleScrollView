 ...
 * 针对项目中需要左右留白进行了简单的处理
 *
 * 如项目中其他位置需要使用不留白的轮播，可直接将 ForSpace 目录下文件拖入
 * SDCycleScrollView 依旧可以使用 pod 管理
 * 修改 LRMagin 控制留白大小
 * 加了圆角, 可在 cellForItemAtIndexPath 中去除
 */


#import "FixSDCycleScrollView.h"
//#import <SDCycleScrollView/SDCollectionViewCell.h>
//#import <SDCycleScrollView/UIView+SDExtension.h>
...
@interface FixSDCycleScrollView () <UICollectionViewDataSource, UICollectionViewDelegate>
...
@implementation FixSDCycleScrollView
...
- (void)initialization
{
    _pageControlAliment = FixSDCycleScrollViewPageContolAlimentCenter;
    ...
    _pageControlStyle = FixSDCycleScrollViewPageContolStyleClassic;
...
{
    FixSDCycleScrollView *cycleScrollView = [[self alloc] initWithFrame:frame];
...
+ (instancetype)cycleScrollViewWithFrame:(CGRect)frame delegate:(id<FixSDCycleScrollViewDelegate>)delegate placeholderImage:(UIImage *)placeholderImage
{
    FixSDCycleScrollView *cycleScrollView = [[self alloc] initWithFrame:frame];
...
- (void)setupMainView
{
    CenterPageCollectionLayout *flowLayout = [[CenterPageCollectionLayout alloc] init];
    flowLayout.minimumLineSpacing = LRMagin;
    flowLayout.scrollDirection = UICollectionViewScrollDirectionHorizontal;
    flowLayout.itemSize = CGSizeMake(self.sd_width-(LRMagin*2), self.sd_height);
    flowLayout.sectionInset = UIEdgeInsetsMake(0, LRMagin, 0, LRMagin);
...
- (void)setDelegate:(id<FixSDCycleScrollViewDelegate>)delegate
{
    _delegate = delegate;
...
- (void)setCurrentPageDotImage:(UIImage *)currentPageDotImage
{
    ...
    if (self.pageControlStyle != FixSDCycleScrollViewPageContolStyleAnimated) {
        self.pageControlStyle = FixSDCycleScrollViewPageContolStyleAnimated;
    }
...
- (void)setPageControlStyle:(FixSDCycleScrollViewPageContolStyle)pageControlStyle
{
...
- (void)setupPageControl
{
    ...
    switch (self.pageControlStyle) {
        case FixSDCycleScrollViewPageContolStyleAnimated:
        {
        ...
        case FixSDCycleScrollViewPageContolStyleClassic:
        {
...
- (void)scrollToIndex:(int)targetIndex
{
    if (targetIndex >= _totalItemsCount) {
        if (self.infiniteLoop) {
            ...
            CGFloat offsetX = targetIndex * (self.sd_width-LRMagin*2) +  (targetIndex) * LRMagin;
            [_mainView setContentOffset:CGPointMake(offsetX, 0) animated:NO];
        }
        return;
    }
    CGFloat offsetX = _mainView.contentOffset.x + (self.sd_width - LRMagin);
    [_mainView setContentOffset:CGPointMake(offsetX, 0) animated:YES];
...
//        [_mainView scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:targetIndex inSection:0] atScrollPosition:UICollectionViewScrollPositionNone animated:NO];
    ...
    if (self.pageControlAliment == FixSDCycleScrollViewPageContolAlimentRight) {
